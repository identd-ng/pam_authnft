...
show_firewall() {
    ...
    echo "#   normally."
    echo "#"
    echo "# SHARED SETS — NO DROP RULES"
    echo "#   All user fragments share the same chain and the same named sets."
    echo "#   A drop rule matching @session_map_ipv4 fires for ANY active"
    echo "#   session whose element is in the set, not just the fragment"
    echo "#   author's. Use accept-only rules; per-session deny-default"
    echo "#   enforcement requires per-session chain isolation."
    echo "#   Integration test 10.13 verifies this behavior."
    echo ""
    ...
    printf "add rule inet authnft %s_egress socket cgroupv2 level 2 @session_map_cg \\\\\n" "$USER_NAME"
