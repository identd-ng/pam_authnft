...
show_setup() {
    ...
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip saddr @session_v4 accept\n"
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip6 saddr @session_v6 accept\n"
    printf "EOF\n"
...
show_firewall() {
    ...
    echo "#   socket cgroupv2 level 2 . ip saddr @session_v4"
    ...
    echo "#   pam_authnft adds 'ct state established,related accept' to the"
    echo "#   shared filter chain ahead of session jump rules. Fragments do"
    echo "#   not need to include it. Pre-scope traffic (the SSH connection,"
    echo "#   DNS lookups sshd made during login, etc.) is accepted before"
    echo "#   the per-session chain is evaluated."
    echo "#"
    echo "# SYN HANDLING"
    ...
    echo "#   Each session gets its own chain (@session_chain) and three"
    echo "#   sets (@session_v4, @session_v6, @session_cg). Rules in the"
    echo "#   per-session chain cannot match another session's traffic."
    echo "#   Drop rules are safe — they only affect the fragment author's"
    echo "#   session. Integration test 10.13 verifies this."
    echo ""

    echo "# ═══════════════════════════════════════════════════════════════════"
    echo "# INBOUND (per-session chain — @session_v4 / @session_v6)"
    echo "# ═══════════════════════════════════════════════════════════════════"
    echo ""
    ...
    printf "# /etc/authnft/users/%s\n" "$USER_NAME"
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip saddr @session_v4 tcp dport { 80, 443 } accept\n"
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip6 saddr @session_v6 tcp dport { 80, 443 } accept\n\n"

    printf "# --- Log and accept ---\n"
    printf "# /etc/authnft/users/%s\n" "$USER_NAME"
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip saddr @session_v4 log prefix \"authnft_%s: \" accept\n" "$USER_NAME"
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip6 saddr @session_v6 log prefix \"authnft_%s: \" accept\n\n" "$USER_NAME"

    printf "# --- Time-restricted access ---\n"
    printf "# /etc/authnft/users/%s\n" "$USER_NAME"
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip saddr @session_v4 \\\\\n"
    printf "    meta day { \"Mon\",\"Tue\",\"Wed\",\"Thu\",\"Fri\" } hour \"09:00\"-\"17:00\" accept\n"
    printf "add rule inet authnft @session_chain socket cgroupv2 level 2 . ip6 saddr @session_v6 \\\\\n"
    printf "    meta day { \"Mon\",\"Tue\",\"Wed\",\"Thu\",\"Fri\" } hour \"09:00\"-\"17:00\" accept\n\n"
    ...
    echo "# ═══════════════════════════════════════════════════════════════════"
    echo "# OUTBOUND (OUTPUT hook — @session_cg, cgroup-only)"
    echo "#"
    echo "# On the output path, the cgroup is the sending socket's and"
    echo "# ip daddr is the destination. Use @session_cg (no src_ip leg)"
    echo "# because ip saddr on output is the host's own address."
    echo "# @session_cg is family-agnostic; add ip daddr / ip6 daddr"
    echo "# variants for dual-stack destinations."
    echo "#"
    echo "# NOTE: egress chains created by fragments are per-user, not"
    echo "# per-session. They persist across sessions and rules accumulate."
    echo "# ═══════════════════════════════════════════════════════════════════"
    echo ""

    printf "# --- Destination logging (bastion host) ---\n"
    printf "# /etc/authnft/users/%s\n" "$USER_NAME"
    printf "# Log outbound connections from session processes to the database tier.\n"
    printf "add chain inet authnft %s_egress { type filter hook output priority filter - 1; }\n" "$USER_NAME"
    printf "add rule inet authnft %s_egress socket cgroupv2 level 2 @session_cg \\\\\n" "$USER_NAME"
    printf "    ip daddr 10.0.5.0/24 log prefix \"authnft_%s_egress: \" accept\n" "$USER_NAME"
    printf "add rule inet authnft %s_egress socket cgroupv2 level 2 @session_cg \\\\\n" "$USER_NAME"
    printf "    ip6 daddr fd00:db::/64 log prefix \"authnft_%s_egress: \" accept\n\n" "$USER_NAME"

    printf "# --- DNS resolver logging (only approved nameservers) ---\n"
    printf "# /etc/authnft/users/%s\n" "$USER_NAME"
    printf "# Log DNS queries from session processes to approved resolvers.\n"
    printf "add chain inet authnft %s_dns { type filter hook output priority filter - 1; }\n" "$USER_NAME"
    printf "add rule inet authnft %s_dns socket cgroupv2 level 2 @session_cg \\\\\n" "$USER_NAME"
    ...
    echo "# ═══════════════════════════════════════════════════════════════════"
    echo "# NAT (POSTROUTING hook — @session_cg, cgroup-only)"
    echo "# ═══════════════════════════════════════════════════════════════════"
    echo ""
    ...
    printf "# Use session_cg (cgroup-only) to match outbound traffic from this session.\n"
    printf "add chain inet authnft %s_nat { type nat hook postrouting priority srcnat; }\n" "$USER_NAME"
    printf "add rule inet authnft %s_nat socket cgroupv2 level 2 @session_cg masquerade\n\n" "$USER_NAME"
    ...
    printf "#\n"
    printf "# Included files do NOT receive placeholder substitution.\n"
    printf "# Use the shared filter chain with accept-only rules in includes.\n"
    printf "#\n"
    printf "# /etc/authnft/groups/sre-base.nft  (shared, root-owned, no placeholders)\n"
    printf "# SRE access: accept-only rules in the shared filter chain.\n"
    printf "add rule inet authnft filter tcp dport { 22, 443, 9090 } accept\n\n"
    printf "# /etc/authnft/users/%s  (break-glass: base + database + Redis + Kafka)\n" "$USER_NAME"
    printf "# Top-level fragment uses placeholders; included file does not.\n"
...
show_monitor() {
    ...
    echo "# Full table state (per-session chains, sets, elements, rules, counters)"
