...
| **zizmor** | `.github/workflows/analyze.yml` (`workflows` job) | every PR | the workflow files themselves: template injection into `run:` blocks, unpinned or stale action pins, over-broad job permissions, credentials left behind by checkout, missing Dependabot cooldown. Policy in `.github/zizmor.yml` |
